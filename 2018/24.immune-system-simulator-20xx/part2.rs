...
fn immune_units(boost: i64, mut groups: Vec<Group>) -> i64 {
    ...
    while sel_ord.iter().any(|&x| groups[x].army == 1) && sel_ord.iter().any(|&x| groups[x].army == 2) {
        ...
        for &i in &sel_ord {
            groups[i].target = (0..groups.len())
                .filter(|&x| groups[x].units > 0 && !groups[x].attacker && groups[x].army != groups[i].army && groups[i].damage_to(&groups[x]) > 0)
        ...
        for &i in &att_ord {
            if let Some(j) = groups[i].target {
                let dead = (groups[i].damage_to(&groups[j]) / groups[j].points).min(groups[j].units);
                groups[j].units -= dead;
                if dead > 0 {
                    attack = true;
                }
    ...
    if sel_ord.iter().any(|&x| groups[x].army == 2) {
        return 0;
    } else {
        return sel_ord.iter().map(|&x| groups[x].units).sum::<i64>();
    }
}
...
fn main() {
    ...
    while last == 0 {
        i += 1;
        last = immune_units(i, groups.clone());
    }
    println!("Result: {}", last);
}
